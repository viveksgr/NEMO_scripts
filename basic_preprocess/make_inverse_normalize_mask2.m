...
% %% For transforming the gmmask:
% matlabbatch = [];
% datapath = fullfile(root, 'imaging', 'nii', 'set_01', 'sess_01', 'run_01');
% n = dir(fullfile(datapath, 'rf*.nii'));
% fimages{1,1} = sprintf('%s,1', fullfile(datapath, n(1).name));
% target = fullfile(pwd,'gmNEMO02.nii');
% matlabbatch{1}.spm.spatial.coreg.write.ref = {fimages{1,1}};
% matlabbatch{1}.spm.spatial.coreg.write.source = {target};
% matlabbatch{1}.spm.spatial.coreg.write.roptions.interp = 0;
% matlabbatch{1}.spm.spatial.coreg.write.roptions.wrap = [0 0 0];
% matlabbatch{1}.spm.spatial.coreg.write.roptions.mask = 0;
% matlabbatch{1}.spm.spatial.coreg.write.roptions.prefix = 'r';
